...
object JsonMapping {
  ...
  implicit val uuidFmt: Format[UUID] = valueFormat(UUID.fromString)(_.toString)
  ...
  class TypeChoiceFormat[R](private val choices: Seq[TypeChoiceMapping[_ <: R]]) extends OFormat[R] {
    {
      ...
      require(overlapping.isEmpty, "overlapping choices are not allowed: " + overlapping.map(pair => pair.head.manifest + " is assignable from " + pair(1).manifest).mkString(", "))
    }
    ...
    override def reads(json: JsValue) = for {
      (name, data) <- ((JsPath \ "type").read[String] ~ (JsPath \ "data").read[JsValue]).tupled.reads(json)
